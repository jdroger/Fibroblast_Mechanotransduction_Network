...
lt_mcc = calculateMCC(act_lt,idx_ecm_pro3,idx_ecm_anti2,idx_ecm_inhib);
ht_mcc = calculateMCC(act_ht,idx_ecm_pro3,idx_ecm_anti2,idx_ecm_inhib);
...
threshold = 0.1;
idx_adaptive_1 = find(lt_mcc <= -threshold & ht_mcc >= threshold); 

% Method 2: rank perturbs based on MCC, find top-ranked perturbs
[~,sort_lt_idx] = sort(lt_mcc,"ascend");   % sort arrays
[~,sort_ht_idx] = sort(ht_mcc,"descend");
...
idx_adaptive = sort_score_idx(1:50,1).';
...
fig_lt = scatter(xdata,lt_mcc,4); hold on
fig_ht = scatter(xdata,lt_mcc,4,"MarkerEdgeAlpha",0.5);
legend('Low Tension','High Tension');
xlabel('Mechano-Adaptive Score'); ylabel('Matrix Content Change');
set(gca,"FontSize",12,"XLim",[0 length(xdata)]);
grid on
