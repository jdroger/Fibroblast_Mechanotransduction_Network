...
c_4a = c_4(1:length(c_4)/2,:);              % 06.16.2020 JR: further divide to get through HPC
% c_4a(c_4a(:,1)==58,:) = [];                 % removing ERK-related combinations
c_4a(c_4a(:,1)~=58,:) = [];                 % choosing ERK-related combinations only
c_4a(c_4a(:,2)<c_error(2),:) = [];          % change to further divide ERK simulations
c_4b = c_4(length(c_4)/2+1:end,:);
...
for choice = choice_all
    for sim = 1:length(tension_all)
        ...
        act_delta = drugscreen_mixedtypes_oe(screentype,inputs,inputNode,tension,choice,c_4a);
        ...
        filename = strcat(screentype,"_tension",tensionname,"_",choice,"_4a_ERK3.mat");
