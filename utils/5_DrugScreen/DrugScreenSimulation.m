function [act_b,act_ko,act_delta] = drugscreen_mixedtypes_oe(screentype,inputlvls,inputnodes,tension,choice,combinations)
% Generated by Netflux 0.08a on 15-Oct-2019
% Function screens changes in network activity for either knockdowns of
% two-node combinations (KO) or overexpression of two nodes (OE). This
% function should be edited to include the parameters and functions from a
% Netflux-generated script, and either "KO" or "OE" should be specified as
% a function argument. 
%
% It is recommended to run this script via highperformance cluster 
% (especially if using a large network), and the 'drugscreen_HPC.m' script 
% can be used for this purpose.
% 
% Edited 05.07.2020 JR: Added inputlvls,inputnodes arguments from fitted
% curves (InputCurve_12_19.m script)
% Edited 05.08.2020 JR: Added mixed perturbation types: "KO-OE", "OE-KO"
% (setPerturbs function handles all 4 types)

%% Validate arguments + set default if needed
isOption = any(contains(["KO","OE","KO-OE","OE-KO"],choice));
if isOption ~= 1
    choice = "KO";
    fprintf("Drug screen type %s not a valid choice; switching to default\n", choice)
else
    fprintf("Drug screen type: %s\n", choice)
end

%% Parameters from Netflux script
% Paste directly from Netflux-generated script
...
% fback = [11:17,141,158,167];                % sets feedback rxn weights manually
% w(fback) = 0.75;
% n(:) = 1.25;                        % change to new n default
% EC50(:) = 0.6;                     % change to new EC50 default
rpar = [w;n;EC50];                          % packages parameters

% set reaction/species variables
...
% if screentype == "combinations"
%     combinations = nchoosek(1:length(speciesNames),2);
% else
%     combinations = nchoosek(1:length(speciesNames),1);
% end

% create storage arrays for basal/KO activity levels
% act_b = zeros(1, length(speciesNames));
act_ko = zeros(size(combinations,1), length(speciesNames));

% run baseline simulation, store in y1
...
% create for loop through all species
for pair = 1:size(combinations,1)
    nodes = combinations(pair,:);
    if screentype == "combinations"
        fprintf("Combination [%u, %u]\n", nodes(1),nodes(2))
    else
        fprintf("Combination [%u]\n", nodes(1))
    end

    % run simulation with double perturbation, store in y2
    ymax_sim = ymax;                              % reset vals    
    ymax_p = setPerturbs(ymax_sim,nodes,screentype,choice);
    params = {rpar,tau,ymax_p,speciesNames};      % integrate into params
    tspan = [80 320];
    [~,y2] = ode23(@ODEfun,tspan,act_b,options,params); % start with y1
    
    % store basal/KO levels
    % act_b(pair,:) = real(y1(end,:));
    act_ko(pair,:) = real(y2(end,:));
end

% calculate delta(Activity): abs(act_ko - act_b)
act_delta = (act_ko - real(act_b));                 % added real fcn for final calculation
...
function ymax_p = setPerturbs(ymax,nodes,screentype,choice)
% default function output
ymax_p = ymax;
% perturbation functions
ko = @(x) 0.1*x;
oe = @(x) 5*x;
% set perturbation types
if choice == "KO"           % double knockout
     perturb1 = ko;
     perturb2 = ko;
elseif choice == "OE"       % double overexpression
    perturb1 = oe;
    perturb2 = oe;
elseif choice == "KO-OE"    % knockout 1 + overexpress 2
    perturb1 = ko;
    perturb2 = oe;
elseif choice == "OE-KO"     % overexpress 1 + knockout 2
    perturb1 = oe;
    perturb2 = ko;
end
ymax_p(nodes(1)) = perturb1(ymax(nodes(1)));
if screentype == "combinations"
    ymax_p(nodes(2)) = perturb2(ymax(nodes(2)));
end