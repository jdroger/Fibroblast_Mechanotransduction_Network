function snm_1_1_rev4_validationsimulations() 
% Generated by Netflux 0.08a on 15-Oct-2019
% Edited 06.04.2020 JR: used final network + fitted params + fback
...
act_b = zeros(length(inputNames), length(speciesNames));
act_s = zeros(length(inputNames), length(speciesNames));

% create for loop through all inputs
for node = 1:length(inputNames)
    progress = node/length(inputNames)*100;
    fprintf("Progress: %.1f%%\n",progress);
    % run baseline simulation, store levels in t1, y1
    % reset w vals from previous iteration
    w_stim = w;
    w_stim(3) = 0.4;
    % w_stim(fback) = 0.8;
    rpar = [w_stim;n;EC50];
    ...
    tspan = [0 80];
    options = [];
    [t1,y1] = ode23(@ODEfun,tspan,y0,options,params);
    
    % run simulation with single stimulus (w=1), store levels in t2, y2
    w_stim(node) = 0.8;
    rpar = [w_stim;n;EC50];
    params = {rpar,tau,ymax,speciesNames};  % integrate new w into params
    tspan = [80 320];
    [t2,y2] = ode23(@ODEfun,tspan,y1(size(y1,1),:),options,params); % start simulation with previous t,y set

    % save baseline/stimulus levels in storage arrays
    act_b(node,:) = real(y1(size(y1,1),:));
    act_s(node,:) = real(y2(size(y2,1),:));
...
act_delta = (act_s - act_b);
% infer qualitative trends based on threshold
predictions = zeros(length(inputNames),length(speciesNames));
threshold = 0.05;
predictions(act_delta<(-1*threshold)) = -1;   % flag decreases
predictions(act_delta>1*threshold) = 1;     % flag increases

% save results
save(strcat(savedir,'snm_1_1_rev4_validation_act_delta.mat'), 'act_delta');
save(strcat(savedir,'snm_1_1_rev4_validation_predictions.mat'), 'predictions');
save(strcat(savedir,'snm_1_1_rev4_validation_inputs.mat'),'inputNames');
save(strcat(savedir,'snm_1_1_rev4_validation_speciesNames.mat'),'speciesNames');
