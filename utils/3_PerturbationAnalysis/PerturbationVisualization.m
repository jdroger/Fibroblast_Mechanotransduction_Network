function PerturbationVisualization
% 04-16-2020 JR
...
for tension = 1:length(tensioncases)
    ...
    load(strcat(filedir,'toplists_tension',tensionname,'.mat'), ...
...
%%%%%% Figure 4D: Knockdown Sensitivity plots across tension cases %%%%%%
...
plotfilename = strcat(plotdir,'4D_sens_allcases.fig');
saveas(fig,plotfilename)
% plotfilename = strcat(plotdir,'4D_sens_allcases.svg');
% saveas(fig,plotfilename)

%%%%%% Figure 4E: Knockdown Influence plots across tension cases %%%%%%
...
plotfilename = strcat(plotdir,'4E_infl_allcases.fig');
saveas(fig,plotfilename)
% plotfilename = strcat(plotdir,'4E_infl_allcases.svg');
% saveas(fig,plotfilename)
...
%%%%%% Figure 4A-C: Top influencer/sensitive node heatmaps %%%%%%
map2 = brewermap(19,'*RdBu');
subplotnames = ["4A","4B","4C"];
idx_tension = 31;
% load act_delta data
...
for tension = 1:length(tensioncases)
    ...
    set(gca,"FontSize",12);
    tensionname = replace(string(tensioncases(tension)),".","");
    plotfilename = strcat(plotdir,subplotnames(tension),'_topnodes_tension',tensionname,'.fig');
    saveas(fig_delta,plotfilename)
    % plotfilename = strcat(plotdir,subplotnames(tension),'_topnodes_tension',tensionname,'.svg');
    % saveas(fig_delta,plotfilename)
end
