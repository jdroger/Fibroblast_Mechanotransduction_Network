function act_delta = PerturbationSimulations(tension) 
% Generated by Netflux 0.08a on 14-Apr-2020
% Edited 04-16-2020 JR: Added 'tension' argument: must be 1x1 double
% Edited 05-19-2020 JR: Added new n/EC50 default values from parameter fit
...
w(3) = tension;                 % sets tension level
...
% fback = [11:17,141,158,167];    % 04.29.2020: set feedback rxn weights manually
% w(fback) = 0.75;
% n(:) = 1.25;
% EC50 (:) = 0.6;
rpar = [w;n;EC50];
...
parfor node = 1:length(speciesNames)
    ...
    % reset ymax vals from previous iteration
    ymax_test = ymax;
    ...
    % run simulation with single KO (ymax=0), store levels in t2, y2
    ymax_test(node) = 0.1*ymax_test(node);
    params = {rpar,tau,ymax_test,speciesNames};  % integrate new ymax into params
    tspan = [80 320];
    [~,y2] = ode23(@ODEfun,tspan,y1(end,:),options,params); % use endpoint y set
    
    % save baseline/KO levels in storage arrays
    act_b(node,:) = real(y1(end,:));
    act_ko(node,:) = real(y2(end,:));
end

% calculate delta(Activity): abs(act_ko - act_b)
act_delta = (act_ko - act_b);
