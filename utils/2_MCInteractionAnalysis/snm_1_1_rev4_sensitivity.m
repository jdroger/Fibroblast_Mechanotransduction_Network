function snm_1_1_rev4_sensitivity() 
% Generated by Netflux 0.08a on 14-Apr-2020
% Edited by JR: 14-Apr-2020
 
savedir = 'D:\Research\Aim2\ModelExpansion\1_1\rev4\SensitivityAnalysis_PrelimData\';
...
% testing for new rxns
% w(170) = 0;

% n(:) = 1.25;
% EC50(:) = 0.6;
% reaction parameters: 
% input = input species selection (AngII, TGFB, IL6, IL1, TNFa, NE, PDGF, 
% ET1, NP = 1, 2, 4, 5, 6, 7, 8, 9, 10)
% mech = relative mechanical activation ([0 1])
% weight = relative input activation ([0 1])

for input = [1:2,4:10]
    fprintf("Starting input %d...\n",input)
    activation = zeros(101,length(speciesNames),6);
    doses = zeros(101,1);
    legendlabels = cell(1,6);
    for mech = 0:8                          % 04.28.2020: additional mech levels
        mech_val = 0.1 + 0.1*mech;
        legendlabels(mech+1) = {strcat("tension = ",string(mech_val))};
        fprintf("Starting tension: %.2f...\n",mech_val)
        for weight = 0:0.01:1
            fprintf("weight = %.2f\n",weight)
            % Run simulation series (input doses 0 to 1)
            w_stim = w;                     % reset weights
            %w_stim(fback) = 0.75;           % 04.28.2020: limit feedback
            w_stim(1,input) = weight;       % set input weight
            w_stim(1,3) = mech_val;         % set tension weight
            rpar = [w_stim;n;EC50];         % lump into params
            params = {rpar,tau,ymax,speciesNames};
            tspan = [0 160];
            options = [];
            
            [~,y] = ode23(@ODEfun,tspan,y0,options,params);     % run ODE 
            row = round((weight*100)+1,0);              % set output array row
            doses(row,1) = weight;                      % assign input dose
            activation(row,1:end,mech+1) = y(end,:);    % store final activation levels
        end
    end
    
    % save activation levels to file
    save(strcat(savedir,cell2mat(inputNames(input)),'_paramfit.mat'),'activation')
    
    % plot dose-response curves across all tension levels
    for j = 82                      % drange(1,1,91)    % select output used
        fig = figure;
        for i = 1:size(activation,3)                    % select tension level
            plot(doses,real(activation(:,j,i)),'LineWidth',2.0);
            hold on
        end
        legend(legendlabels, "Location","northeastoutside");
        xlabel(strcat(cell2mat(inputNames(input)),' Dose'));
        ylabel('proCI Fractional Activation');
        title(speciesNames(j));
        hold off
        % plotfile = strcat(cell2mat(inputNames(input)),'_fback_',cell2mat(speciesNames(j)));
        % plotfilename = strcat(savedir,plotfile,'.fig');
...
dydt(Rho) = (OR(act(y(TGFB1R),rpar(:,118)),AND(rpar(:,131),act(y(RhoGEF),rpar(:,131)),inhib(y(RhoGDI),rpar(:,131))))*ymax(Rho) - y(Rho))/tau(Rho); 
